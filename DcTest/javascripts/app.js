...
$('#member .member').on('mouseenter', function(){
  $(this).animate({height:'400px'},'fast');
})
$('#member .member').on('mouseleave', function(){
...
$('.success-story section h3').on('click',function(){
  var selStory = $(this);
  console.log('top: ' + selStory.position().top)
  console.log('hi');
  var info = $(this).next('div').html();
  console.log(info)
  var openMe = $(this).parent('.row').next('.story-info');
  console.log(openMe)
  if($('.oppened').length > 0){
    console.log('hi2');
    $('.oppened').animate({height:'0px'},300, function(){
      openMe.html(info); 
      console.log('hi')
      $(this).removeClass('oppened');
      $('h3').removeClass('oppened-heading');
      // $('.active-arrow').remove();
      //selStory.css('background','url(../images/index3.png) 10px -6px no-repeat #c02');
      //selStory.css('height','200px');
      curHeight = openMe.height(),
      autoHeight = openMe.css('height', 'auto').height();
      openMe.height(curHeight).animate({height: autoHeight},'fast', function(){
        $('html, body').animate({
        'scrollTop': $(openMe).offset().top - 400
      }, 'fast');
        openMe.addClass('oppened')
        selStory.addClass('oppened-heading');
        //selStory.css('background','url(../images/active-story.png)');
        //selStory.css('height','250px');
        //selStory.append("<span class='active-arrow'></span>");
      });
    })
  }else{
    openMe.html(info);
